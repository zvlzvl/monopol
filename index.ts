...
let allCards: (null | CardInterface)[] = [
    {
        "id": "1",
        "place": "corner",
        "name": "GO",
...
const board = document.querySelector(".monopoly-board") as HTMLDivElement;
boardHTML();
const turn = document.querySelector(".turn") as HTMLDivElement;
const cells = document.querySelectorAll(".cell") as NodeListOf<HTMLDivElement>;
const firstPlayerDiv = document.querySelector(".player1-cards") as HTMLDivElement;
const secondPlayerDiv = document.querySelector(".player2-cards") as HTMLDivElement;
const actionDiv = document.querySelector(".action") as HTMLDivElement;


let whoGo: string = "player1";
...
let cardOpen: CardInterface;

addPlayers();
addPlayerProperty();

const dice = document.querySelector(".dice") as HTMLDivElement;
const diceArr: number[] = [1, 2, 3, 4, 5, 6];
dice.onclick = () => {
    if (canRoll) {
        let numToGo: number = rollDice();
...
function showCard(id: number):void {
    // @ts-ignore
...
function tax(nowPlaying: PlayerInterface, cardOpen: CardInterface):void {
    actionDiv.innerHTML = `
    <div class="img img-tax"> 
        <h1>${cardOpen.name}</h1>
        <button>Pay ${cardOpen.amount}</button>
    </div>`
    const payTax = document.querySelector(".img-tax button") as HTMLButtonElement;
    payTax.onclick = () => {
...
function goJail(nowPlaying: PlayerInterface):void {
    setTimeout(() => {
        nowPlaying.on = 11;
        addPlayers();
        whoGo = whoGo ==="player1" ? "player2" : "player1";
...
function railroad(nowPlaying: PlayerInterface, cardOpen: CardInterface, otherPlayer: PlayerInterface):void {
    let canBuy: boolean = !cardOpen.sold;
    // @ts-ignore
    let enoughMoney:boolean = (nowPlaying.money - cardOpen.price) >= 0

    if (canBuy && enoughMoney) {
        let count: null | number = cardOpen.price;
        actionDiv.innerHTML = `
            ...
            `
        const buyRail = document.querySelector(".img-rail .buy") as HTMLButtonElement;
        const skip = document.querySelector(".img-rail .skip") as HTMLButtonElement;
        buyRail.onclick = () => {
            if (!cardOpen.sold) {
                // @ts-ignore
        ...
        let ownerName: string | boolean = cardOpen.sold;

        if (ownerName !== nowPlaying.name) {
            // @ts-ignore
            let count: number = countRailAmount(otherPlayer, cardOpen.rent);
            actionDiv.innerHTML = `
            <div class="img img-rail flex j-center a-center gap">
            <div class="buttons"> 
        ...
        `
            const payRail = document.querySelector(`#pay-${cardOpen.id}`) as HTMLButtonElement;
            payRail.onclick = () => {
...
function electric(nowPlaying: PlayerInterface, cardOpen: CardInterface, otherPlayer: PlayerInterface):void {
    let canBuy: boolean = !cardOpen.sold;
    // @ts-ignore
    let enoughMoney:boolean = (nowPlaying.money - cardOpen.price) >= 0
    ...
    if (canBuy && enoughMoney) {
        ...
        actionDiv.innerHTML = `
            ...
            `
        const buyElectric = document.querySelector(".img-electric .buy") as HTMLButtonElement;
        const skip = document.querySelector(".img-electric .skip") as HTMLButtonElement;
        buyElectric.onclick = () => {
            if (!cardOpen.sold) {
                // @ts-ignore
                let findCard = cards.find(item => item.id === String(cardOpen.id));
        ...
        if (ownerName !== nowPlaying.name) {
            // @ts-ignore
            let count: number = countUtilityAmount(otherPlayer, cardOpen.rent);
            actionDiv.innerHTML = `
            <div class="img img-electric flex j-center a-center gap">
        ...
        `
            const payWater = document.querySelector(`#pay-${cardOpen.id}`) as HTMLButtonElement;
            payWater.onclick = () => {
                let pay: number = countHouses(nowPlaying);
...
function water(nowPlaying: PlayerInterface, cardOpen: CardInterface, otherPlayer: PlayerInterface):void {
    let canBuy: boolean = !cardOpen.sold;
    // @ts-ignore
    let enoughMoney:boolean = (nowPlaying.money - cardOpen.price) >= 0
    ...
    if (canBuy && enoughMoney) {
        ...
        actionDiv.innerHTML = `
            ...
            `
        const buyWater = document.querySelector(".img-water .buy") as HTMLButtonElement;
        const skip = document.querySelector(".img-water .skip") as HTMLButtonElement;
        buyWater.onclick = () => {
            if (!cardOpen.sold) {
                // @ts-ignore
                let findCard = cards.find(item => item.id === String(cardOpen.id));
...
function street(nowPlaying: PlayerInterface, cardOpen: CardInterface, otherPlayer: PlayerInterface):void {
    let canBuy: boolean = !cardOpen.sold;
    // @ts-ignore
    let enoughMoney:boolean = (nowPlaying.money - cardOpen.price) >= 0
    if (canBuy && enoughMoney) {
    ...
    `
        const buyStreet = document.querySelector(".buy-street") as HTMLButtonElement;
        ...
        buyStreet.onclick = () => {
            ...
            let newStreet: StreetInterface = {"id": cardOpen.id, "house": 0, "hotels": 0};
    ...
    `
        ...
        if (ownerName !== nowPlaying.name) {
            // @ts-ignore
            let pay: number = countStreetAccount(cardOpen, otherPlayer);
            actionDiv.innerHTML = `
            <div style="border-top${cardOpen.color}">
            <h1>${cardOpen.name}</h1>
            <div>${cardOpen.price}$</div>
        ...
        `
            const payStreet = document.querySelector(".pay-street") as HTMLButtonElement;
            payStreet.onclick = () => {
                nowPlaying.money -= pay;
                otherPlayer.money += pay;
                addPlayerProperty();
        ...
        if(ownerName === nowPlaying.name){
            let housePrice: null | number = cardOpen.houseCost;
            let hotelPrice: null | number = cardOpen.hotelCost;
            let street:StreetInterface;
            let hasHouse:number=0;
            let hasHotel:number=0;
        ...
        `
                const buyHouse = document.querySelector(".buyHouse") as HTMLButtonElement;
                buyHouse.onclick = () => {
                    // @ts-ignore
                    street.house++;
                    // @ts-ignore
        ...
        <div style="border-top${cardOpen.color}">
            ...
            <div class="buttons">
               ...
               <button class="skip">Skip</button>
                ...
                const buyHotel = document.querySelector(".buyHotel") as HTMLButtonElement;
                buyHotel.onclick = () => {
                    console.log(street)
                    street.house = 0;
                    street.hotels=1;
...
function boardHTML():void {
    allCards.forEach((card: CardInterface | null) => {
        if (card === null) {
...
function move(numToGo: number, player: PlayerInterface):void {
...
function clearShowCard():void {
    actionDiv.innerHTML = "";
}

function countStreetAccount(cardOpen:CardInterface, otherPlayer:PlayerInterface):number{
    let streetId = cardOpen.id;
    let amount = 0;
    otherPlayer.streets.forEach((street) => {
        if (street.id === streetId) {
            let house: number = street.house;
            let hotels: number = street.hotels;
            if(hotels > 0){
...
function countUtilityAmount(owner: PlayerInterface, rent: number[]): number {
    return rent[owner.utility.length - 1];
}
function countRailAmount(owner: PlayerInterface, rent: number[]): number {
    return rent[owner.railroad.length - 1];
}
function countHouses(nowPlaying: PlayerInterface): number {
    let houses: number = 0;
